...
describe("BodyProfileService", () => {
	...
	describe("createBodyProfile", () => {
		it("should create body profile successfully", async () => {
			...
			const result = await bodyProfileService.createBodyProfile(
				auth0Id,
				profileData,
			);
			...
			expect(
				bodyProfileService.bodyProfileRepository.create,
			).toHaveBeenCalledWith(userId, profileData);
		});
		...
		it("should throw ApiError when user not found", async () => {
			...
			await expect(
				bodyProfileService.createBodyProfile(auth0Id, profileData),
	...
	describe("updateBodyProfile", () => {
		it("should update body profile successfully", async () => {
			...
			const result = await bodyProfileService.updateBodyProfile(
				auth0Id,
				updateData,
			);
			...
			expect(
				bodyProfileService.bodyProfileRepository.upsert,
		...
		it("should throw ApiError when user not found", async () => {
			...
			await expect(
				bodyProfileService.updateBodyProfile(auth0Id, updateData),
			).rejects.toThrow(ApiError);
			await expect(
				bodyProfileService.updateBodyProfile(auth0Id, updateData),
			).rejects.toThrow("User not found");
	...
	describe("deleteBodyProfile", () => {
		it("should delete body profile successfully", async () => {
			...
			expect(
				bodyProfileService.bodyProfileRepository.delete,
		...
		it("should throw ApiError when user not found", async () => {
			...
			await expect(
				bodyProfileService.deleteBodyProfile(auth0Id),
			).rejects.toThrow(ApiError);
			await expect(
				bodyProfileService.deleteBodyProfile(auth0Id),
			).rejects.toThrow("User not found");
