...
import { Switch as RNSwitch, TouchableOpacity, View } from "react-native";
	...
	return (
		<View>
			<PSSelect
				...
				{...props}
			>
				<Text color={selectedOption ? "$color" : "$placeholderColor"}>
...
export const PSCheckboxComponent: React.FC<PSCheckboxProps> = ({
	...
	return (
		<TouchableOpacity
			...
			disabled={disabled}
		>
			<PSCheckbox checked={checked} size={size} disabled={disabled}>
				{checked && <PSIcon name="check" size="small" />}
			</PSCheckbox>
			{label && (
				<Text color={disabled ? "$placeholderColor" : "$color"}>{label}</Text>
			)}
		</TouchableOpacity>
	);
};
...
export const PSSwitchComponent: React.FC<PSSwitchProps> = ({
	...
	return (
		<RNSwitch
			...
			disabled={disabled}
			trackColor={{
				false: theme.backgroundPress?.val || "#767577",
				true: theme.primary?.val || "#E14F5A",
			}}
			thumbColor={
				value
					? theme.color?.val || "#f5dd4b"
					: theme.background?.val || "#f4f3f4"
			}
