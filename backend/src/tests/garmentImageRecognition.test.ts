...
const garmentServiceModule = jest.requireActual(
	"../services/garmentImageRecognitionService",
) as any;
...
describe("GarmentImageRecognitionService", () => {
	...
	describe("analyzeGarmentImage", () => {
		it("should analyze garment image successfully with base64 input", async () => {
			const mockClaudeResponse = JSON.stringify({
				...
				suggestions: {
					...
					styling_tips: [
						...
						"Layer under jacket for professional look",
					],
				},
			});
			...
			expect(mockAnalyzeImageWithVision).toHaveBeenCalledWith({
				...
				prompt: expect.stringContaining(
					"You are an expert fashion AI assistant",
				),
				isBase64: true,
			});
		});
		...
		it("should handle Claude API errors gracefully", async () => {
			mockAnalyzeImageWithVision.mockRejectedValue(
		...
		it("should include user preferences in analysis prompt", async () => {
			mockAnalyzeImageWithVision.mockResolvedValue(
				'{"confidence": 50, "category": "tops"}',
			);
			...
			expect(callArgs.prompt).toContain(
				"User's style preferences: casual, minimalist",
			);
			expect(callArgs.prompt).toContain(
				"User's color preferences: blue, white, black",
			);
		});
	});
	...
	describe("suggestGarmentName", () => {
		it("should generate name suggestions based on analysis result", async () => {
			...
			const suggestions =
				await imageRecognitionService.suggestGarmentName(analysisResult);
	...
	describe("parseClaudeResponse", () => {
		it("should parse valid JSON response", () => {
			...
			const validResponse =
				'Some text before {"confidence": 90, "category": "tops"} some text after';
