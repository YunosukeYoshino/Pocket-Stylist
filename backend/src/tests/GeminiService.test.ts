...
const geminiServiceModule = jest.requireActual(
...
describe("GeminiService", () => {
	...
	describe("generateStylingRecommendations", () => {
		...
		it("should generate styling recommendations successfully", async () => {
			...
			expect(mockGenerateContent).toHaveBeenCalledWith(
				expect.stringContaining("professional stylist"),
			);
		});
	...
	describe("analyzeUserStyleProfile", () => {
		...
		it("should analyze user style profile successfully", async () => {
			...
			expect(mockGenerateContent).toHaveBeenCalledWith(
				expect.stringContaining("style profile"),
			);
		});
		...
		it("should handle empty garment history", async () => {
			...
			await geminiService.analyzeUserStyleProfile(
				"user-123",
				mockUserPreferences,
				[],
			);
			...
			expect(mockGenerateContent).toHaveBeenCalledWith(
				expect.stringContaining("Garment History:"),
			);
		});
		...
		it("should handle API errors during style analysis", async () => {
			...
			await expect(
				geminiService.analyzeUserStyleProfile(
					"user-123",
					mockUserPreferences,
					mockGarmentHistory,
				),
	...
	describe("generateOutfitDescription", () => {
		const mockApiResponse = {
			response: {
				text: () =>
					"This elegant ensemble perfectly captures the essence of professional spring styling with its sophisticated color palette and versatile pieces that transition seamlessly from office meetings to casual lunch dates.",
			},
		};
		...
		it("should generate outfit description successfully", async () => {
			...
			expect(mockGenerateContent).toHaveBeenCalledWith(
				expect.stringContaining(
					"Create a compelling outfit description for a work occasion in spring",
				),
			);
		});
		...
		it("should include garment IDs in prompt", async () => {
			...
			await geminiService.generateOutfitDescription(
				["garment-1", "garment-2"],
				"casual",
				"summer",
			);
	...
	describe("analyzeImageWithVision", () => {
		const mockApiResponse = {
			response: {
				text: () =>
					"This is a blue denim jacket with silver buttons, classic collar, and regular fit. The fabric appears to be medium-weight cotton denim with slight distressing around the edges.",
			},
		};
	...
	describe("utility methods", () => {
		...
		it("should estimate tokens reasonably", () => {
			...
			const longText =
				"This is a much longer text that should have more estimated tokens";
	...
	describe("error handling", () => {
		...
		it("should handle empty responses gracefully", async () => {
			...
			await expect(
				geminiService.generateOutfitDescription(
					["garment-1"],
					"casual",
					"spring",
				),
