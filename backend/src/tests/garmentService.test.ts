...
describe("GarmentService", () => {
	...
	describe("createGarment", () => {
		...
		it("should throw error for invalid subcategory", async () => {
			await expect(
				...
				"Invalid subcategory: invalid-subcategory for category: tops",
			);
		});
	});
	...
	describe("getGarmentById", () => {
		it("should return garment if found", async () => {
			...
			const result = await garmentService.getGarmentById(
				mockUserId,
				mockGarmentId,
			);
	...
	describe("updateGarment", () => {
		it("should update garment successfully", async () => {
			...
			const result = await garmentService.updateGarment(
				mockUserId,
				mockGarmentId,
				{
					name: "Updated Blue T-Shirt",
	...
	describe("toggleFavorite", () => {
		it("should toggle favorite status", async () => {
			...
			mockPrismaClient.garment.findFirst.mockResolvedValue(mockGarment);
			mockPrismaClient.garment.updateMany.mockResolvedValue({ count: 1 });
			mockPrismaClient.garment.findFirst.mockResolvedValueOnce(
				mockUpdatedGarment,
			);

			const result = await garmentService.toggleFavorite(
				mockUserId,
				mockGarmentId,
			);
