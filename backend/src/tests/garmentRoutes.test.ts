...
describe("Garment Routes", () => {
	...
	describe("GET /api/v1/garments", () => {
		...
		it("should handle search filters", async () => {
			...
			await request(app).get("/api/v1/garments").query({
				search: "blue shirt",
				brand: "Uniqlo",
				priceMin: "20",
				priceMax: "50",
		...
		it("should return 401 for unauthorized requests", async () => {
			...
			const response = await request(unauthorizedApp).get("/api/v1/garments");
	...
	describe("GET /api/v1/garments/:id", () => {
		it("should return specific garment", async () => {
			...
			expect(mockGarmentService.getGarmentById).toHaveBeenCalledWith(
				mockUserId,
				mockGarmentId,
			);
		});
	...
	describe("GET /api/v1/garments/categories", () => {
		it("should return all categories and related data", async () => {
			const response = await request(app).get("/api/v1/garments/categories");
	...
	describe("GET /api/v1/garments/statistics", () => {
		it("should return garment statistics", async () => {
			...
			const response = await request(app).get("/api/v1/garments/statistics");
			...
			expect(mockGarmentService.getGarmentStatistics).toHaveBeenCalledWith(
				mockUserId,
			);
		});
	});
	...
	describe("POST /api/v1/garments/:id/favorite", () => {
		it("should toggle favorite status", async () => {
			...
			const response = await request(app).post(
				`/api/v1/garments/${mockGarmentId}/favorite`,
			);
			...
			expect(mockGarmentService.toggleFavorite).toHaveBeenCalledWith(
				mockUserId,
				mockGarmentId,
			);
		});
	...
	describe("POST /api/v1/garments/analyze-image", () => {
		it("should analyze garment image successfully", async () => {
			...
			mockImageRecognitionService.analyzeGarmentImage.mockResolvedValue(
				mockAnalysisResult,
			);
			...
			expect(
				mockImageRecognitionService.analyzeGarmentImage,
	...
	describe("POST /api/v1/garments/extract-colors", () => {
		it("should extract color palette successfully", async () => {
			...
			mockImageRecognitionService.extractColorPalette.mockResolvedValue(
				mockColorPalette,
			);
	...
	describe("POST /api/v1/garments/suggest-names", () => {
		it("should suggest garment names", async () => {
			...
			mockImageRecognitionService.suggestGarmentName.mockResolvedValue(
				mockSuggestions,
			);
			...
			expect(
				mockImageRecognitionService.suggestGarmentName,
			).toHaveBeenCalledWith(analysisResult);
		});
