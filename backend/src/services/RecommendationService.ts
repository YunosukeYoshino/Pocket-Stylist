...
export class RecommendationService {
		...
		try {
			...
			try {
				...
				const geminiResponse =
					await this.gemini.generateStylingRecommendations(geminiInput);
				...
				await this.prisma.aiRecommendation.update({
					...
					data: {
						...
						personalizationInsights:
							geminiResponse.personalization_insights as any,
		...
		try {
			...
			if (recommendation.userId !== userId) {
				throw new RecommendationError(
					"Unauthorized access to recommendation",
					403,
				);
			}
			...
			await this.updateStyleProfileFromFeedback(
				userId,
				feedbackType,
				rating,
				comment,
	...
	private async prepareGeminiInput(
		...
		if (request.context?.garmentIds) {
			garments = garments.filter((g: any) =>
				request.context?.garmentIds?.includes(g.id),
			);
		}
		if (request.context?.excludeGarmentIds) {
			garments = garments.filter(
				(g: any) => !request.context?.excludeGarmentIds?.includes(g.id),
			);
		}
		...
		return {
			...
			userProfile: {
				...
				preferences: {
					style:
						userStyleProfile.dominantStyles?.[0] || user.preferences?.style,
	...
	private async updateUserStyleProfile(
		userId: string,
		geminiResponse: any,
	): Promise<void> {
		...
		try {
			await this.prisma.userStyleProfile.upsert({
				...
				update: {
					...
					colorPreferences:
						personalizationInsights?.stylePreferencesLearned || [],
					bodyTypeConsiderations:
						personalizationInsights?.bodyTypeConsiderations || [],
	...
	private calculateOverallConfidence(outfits: any[]): number {
		...
		const totalConfidence = outfits.reduce(
			(sum, outfit) => sum + outfit.confidence,
			0,
		);
	...
	public async getCollaborativeRecommendations(userId: string): Promise<any[]> {
		try {
			...
			const recommendations = await this.getOutfitsFromSimilarUsers(
				similarUsers,
				userId,
			);
		...
		try {
			...
			if (existingProfile && !forceRefresh) {
				const hoursSinceLastAnalysis = existingProfile.lastAnalyzed
					? (Date.now() - existingProfile.lastAnalyzed.getTime()) /
						(1000 * 60 * 60)
		...
		try {
			...
			throw error instanceof RecommendationError
				...
				: new RecommendationError(
						"Failed to get weather-based recommendations",
					);
		}
	}
