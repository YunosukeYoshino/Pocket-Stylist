...
import { logger, logRecommendationGeneration } from '../utils/logger';
import { ClaudeService, StylingRecommendationInput } from './ClaudeService';
...
export interface RecommendationRequest {
  userId: string;
  type: 'styling_recommendations' | 'outfit_suggestions' | 'seasonal_updates' | 'trend_analysis' | 'color_matching' | 'body_type_optimization';
  context?: {
    occasion?: string;
    season?: string;
    weather?: string;
...
export class RecommendationService {
  ...
  public async generateRecommendations(request: RecommendationRequest): Promise<RecommendationResponse> {
    ...
    try {
      ...
      throw error instanceof RecommendationError ? error : new RecommendationError('Failed to generate recommendations');
    }
  }
    ...
    try {
      ...
      const response: RecommendationResponse = {
        ...
        outfits: recommendation.items.map((item: any) => ({
          ...
          items: Array.isArray(item.garmentIds) ? (item.garmentIds as string[]).map((id: string, index: number) => ({
            garmentId: id,
            category: 'unknown',
  ...
  public async submitFeedback(
    ...
    try {
      ...
      throw error instanceof RecommendationError ? error : new RecommendationError('Failed to submit feedback');
    }
  }

  private async prepareClaudeInput(user: any, request: RecommendationRequest): Promise<StylingRecommendationInput> {
  ...
  private async getOutfitsFromSimilarUsers(_userIds: string[], _excludeUserId: string): Promise<any[]> {
  ...
  public async analyzeUserStyle(userId: string, forceRefresh: boolean = false): Promise<any> {
    try {
      ...
      throw error instanceof RecommendationError ? error : new RecommendationError('Failed to analyze user style');
    }
  }

  public async getCurrentTrends(season?: string, category?: string, limit: number = 10): Promise<any[]> {
    try {
  ...
  public async generateColorPalette(userId: string): Promise<any> {
    try {
      ...
      throw error instanceof RecommendationError ? error : new RecommendationError('Failed to generate color palette');
    }
  }
  ...
  public async getWeatherBasedRecommendations(
    userId: string,
    location: string,
    includeWeatherData: boolean = true
  ): Promise<any> {
    try {
      ...
      throw error instanceof RecommendationError ? error : new RecommendationError('Failed to get weather-based recommendations');
    }
  }
