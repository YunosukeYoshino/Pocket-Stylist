...
export class GarmentImageRecognitionService {
	...
	private buildAnalysisPrompt(
		userPreferences?: ImageRecognitionInput["userPreferences"],
	): string {
		const categories = GARMENT_CATEGORIES.map(
			(cat) =>
				`${cat.id} (${cat.name}): ${cat.subcategories.map((sub) => sub.id).join(", ")}`,
		).join("\n");

		const colors = GARMENT_COLORS.map(
			(color) => `${color.name} (${color.nameJa}) - ${color.hex}`,
		).join(", ");
...
Respond with only the JSON object, no additional text.`;
	...
	private validateAndNormalizeResult(
		...
		if (
			result.subcategory &&
			isValidSubcategory(normalized.category, result.subcategory)
		) {
		...
		if (result.suggestions) {
			...
			if (Array.isArray(result.suggestions.alternative_categories)) {
				normalized.suggestions.alternative_categories =
					result.suggestions.alternative_categories
						.filter((alt) => alt.category && isValidCategory(alt.category))
						.map((alt) => ({
							category: alt.category,
							subcategory:
								alt.subcategory &&
								isValidSubcategory(alt.category, alt.subcategory)
									? alt.subcategory
									: undefined,
			...
			if (Array.isArray(result.suggestions.alternative_colors)) {
				normalized.suggestions.alternative_colors =
					result.suggestions.alternative_colors
			...
			if (Array.isArray(result.suggestions.care_instructions)) {
				normalized.suggestions.care_instructions =
					result.suggestions.care_instructions
						.filter(
							(instruction) =>
	...
	async extractColorPalette(
		input: ImageRecognitionInput,
	): Promise<Array<{ color: string; hex: string; percentage: number }>> {
		try {
	...
	async suggestGarmentName(
		...
		if (analysisResult.category && analysisResult.subcategory) {
			const category = GARMENT_CATEGORIES.find(
				(cat) => cat.id === analysisResult.category,
			);
			const subcategory = category?.subcategories.find(
				(sub) => sub.id === analysisResult.subcategory,
			);
			...
			if (subcategory) {
				...
				if (analysisResult.color && analysisResult.style) {
					suggestions.push(
						`${analysisResult.color} ${analysisResult.style} ${subcategory.name}`,
					);
				}
			}
		}
