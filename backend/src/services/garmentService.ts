...
export class GarmentService {
	...
	async createGarment(
		...
		if (
			garmentData.subcategory &&
			!isValidSubcategory(garmentData.category, garmentData.subcategory)
		) {
			throw new Error(
				`Invalid subcategory: ${garmentData.subcategory} for category: ${garmentData.category}`,
			);
		}

		// Validate material
		if (
			garmentData.material &&
			!GARMENT_MATERIALS.includes(garmentData.material)
		) {
		...
		try {
			...
			return {
				...garment,
				price: garment.price ? Number(garment.price) : null,
			} as Garment;
		} catch (error) {
	...
	async updateGarment(
		...
		if (
			updateData.material &&
			!GARMENT_MATERIALS.includes(updateData.material)
		) {
		...
		try {
			const garment = await this.prisma.garment.updateMany({
				...
				data: {
					...
					...(updateData.condition !== undefined && {
						condition: updateData.condition,
					}),
					...(updateData.isFavorite !== undefined && {
	...
	): Promise<PaginatedGarmentsResult> {
		const {
			userId,
			filters = {},
			sortBy = "createdAt",
		...
		try {
			...
			garments.forEach((garment: any) => {
				...
				if (garment.condition) {
					conditionCounts[garment.condition] =
						(conditionCounts[garment.condition] || 0) + 1;
				}
			});
