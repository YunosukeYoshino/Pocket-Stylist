  ...
  public async generateStylingRecommendations(
    ...
    try {
      ...
      logClaudeUsage(
        ...
        this.calculateCost(this.estimateTokens(prompt), this.estimateTokens(JSON.stringify(result))),
        processingTime
      )

      return result
    } catch (error) {
      logger.error('Gemini API error in generateStylingRecommendations:', error)
  ...
  public async generateOutfitDescription(
    ...
    try {
      ...
      logClaudeUsage(
        ...
        'outfit_description',
        this.estimateTokens(prompt),
        this.calculateCost(this.estimateTokens(prompt), this.estimateTokens(result)),
        processingTime
      )
  ...
  public async analyzeImageWithVision(input: {
    ...
    try {
      ...
      logClaudeUsage(
        ...
        this.calculateCost(this.estimateTokens(input.prompt), this.estimateTokens(result)),
        processingTime
      )
...
Keep it engaging and professional.`
  ...
  private parseStylingResponse(response: GenerateContentResult): StylingRecommendationOutput {
    try {
      ...
      throw new GeminiAPIError('Failed to parse styling recommendation response')
    }
  }
  ...
  public hashInput(input: any): string {
    const hash = Buffer.from(JSON.stringify(input)).toString('base64').replace(/[^a-zA-Z0-9]/g, '')
