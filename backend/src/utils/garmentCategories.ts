...
export function getAllSubcategories(): Array<
	GarmentSubcategory & { categoryId: string }
> {
...
export function searchCategories(query: string): GarmentCategory[] {
	...
	return GARMENT_CATEGORIES.filter(
		(category) =>
...
export function getTagsForCategory(categoryId: string): string[] {
	...
	const allTags = category.subcategories.flatMap(
