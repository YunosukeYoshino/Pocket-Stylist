...
router.patch(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		if (
			!req.body.garmentIds ||
			!Array.isArray(req.body.garmentIds) ||
			req.body.garmentIds.length === 0
		) {
...
router.patch(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const garment = await garmentService.updateGarment(
			userId,
			req.params.id!,
			updateData,
		);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		try {
			const nameSuggestions = await imageRecognitionService.suggestGarmentName(
