...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const recommendations =
			await recommendationService.generateRecommendations(validatedData);
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const recommendation =
			await recommendationService.getRecommendation(recommendationId);
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const styleProfile =
			await recommendationService.getUserStyleProfile(userId);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const analysis = await recommendationService.analyzeUserStyle(
			userId,
			forceRefresh,
		);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		logger.info("Generating outfit description", {
			garmentIds,
			occasion,
			season,
		});

		const description = await geminiService.generateOutfitDescription(
			garmentIds,
			occasion,
			season,
		);
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const trends = await recommendationService.getCurrentTrends(
			season,
			category,
			limit,
		);
...
router.get(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		const colorPalette =
			await recommendationService.generateColorPalette(userId);
...
router.post(
	...
	asyncHandler(async (req: Request, res: Response) => {
		...
		logger.info("Generating weather-based recommendations", {
			userId,
			location,
		});

		const recommendations =
			await recommendationService.getWeatherBasedRecommendations(
				userId,
				location,
				includeWeatherData,
			);
