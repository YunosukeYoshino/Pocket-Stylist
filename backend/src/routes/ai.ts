...
import { logger, logUserFeedback } from '../utils/logger';
...
const stylingRecommendationSchema = z.object({
  ...
  type: z.enum(['styling_recommendations', 'outfit_suggestions', 'seasonal_updates', 'trend_analysis', 'color_matching', 'body_type_optimization']),
  context: z.object({
...
const feedbackSchema = z.object({
  userId: z.string().uuid(),
  outfitId: z.string(),
  feedbackType: z.enum(['like', 'dislike', 'favorite', 'rating', 'comment', 'worn', 'purchased', 'skipped']),
  rating: z.number().min(1).max(5).optional(),
  comment: z.string().max(500).optional(),
});

// POST /v1/ai/styling-recommendations
router.post('/styling-recommendations', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/recommendations/:id', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/user-profile/:userId', asyncHandler(async (req: Request, res: Response) => {
...
router.post('/analyze-style', asyncHandler(async (req: Request, res: Response) => {
...
router.get('/collaborative-recommendations/:userId', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.params.userId;
...
router.get('/content-based-recommendations/:userId', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.params.userId;
...
router.post('/generate-outfit-description', asyncHandler(async (req: Request, res: Response) => {
  const schema = z.object({
...
router.get('/color-palette/:userId', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.params.userId;
...
router.get('/recommendation-history/:userId', asyncHandler(async (req: Request, res: Response) => {
  const userId = req.params.userId;
  const query = z.object({
    page: z.string().transform(val => parseInt(val, 10)).default('1'),
    limit: z.string().transform(val => parseInt(val, 10)).default('10'),
    type: z.enum(['styling_recommendations', 'outfit_suggestions', 'seasonal_updates', 'trend_analysis', 'color_matching', 'body_type_optimization']).optional(),
  });
...
router.post('/weather-based-recommendations', asyncHandler(async (req: Request, res: Response) => {
  const schema = z.object({
    userId: z.string().uuid(),
